...
import { getCompatibleNamespaces, readToolLockFromItem, registerToolLockResolver } from './toolMemory.js'
...
const FURNITURE_HAMMER_COMPONENT_ID = 'dorios_atelier:furniture_hammer'
const FURNITURE_HAMMER_ID_SUFFIX = '_furniture_hammer'

// Lock metadata and variant state mapping.
const HAMMER_LOCK_KIND = 'hammer'
const STATE_ORDER = ['default', 'stairs', 'slab', 'vertical_slab', 'wall']
const STATE_SUFFIX = {
    default: '',
    stairs: '_stairs',
    slab: '_slab',
...
system.beforeEvents.startup.subscribe(initEvent => {
    const handlers = {
        onUseOn(event) {
            ...
            if (source?.isSneaking) {
                ...
                } else if (rawName.endsWith('_wall')) {
                    currentState = 'wall'
                    baseName = rawName.slice(0, -'_wall'.length)
    ...
    try {
        initEvent.itemComponentRegistry.registerCustomComponent(FURNITURE_HAMMER_COMPONENT_ID, handlers)
    } catch {
